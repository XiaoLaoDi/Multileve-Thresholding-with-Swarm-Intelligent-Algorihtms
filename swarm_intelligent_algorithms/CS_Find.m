function  CS_result=CS_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
% function CS_result=CS_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
...
%    MAX_Iterations:
% Output:
%    CS_result: is a 'Struct' containing the following result information
%        CS_result.Fitness:
%            CS_result.Fitness.Mean
%            CS_result.Fitness.Variance
%            CS_result.Fitness.Max
%            CS_result.Fitness.Min
%        CS_result.Success_Rate=sum(Success_FindBest_Num)/RunTimes;
%        CS_result.BestThresholds: a vector containing 'TH_num' values
%        CS_result.EachRunConvergenceTime:a vector containing 'RunTimes' values
%        CS_result.MeanConvergenceTime: the mean convergence time

disp('the Cuckoo Search is running...')

%% Cuckoo Search STEPS
    %% STEP ONE
        % Initialize the CS parameters
...
%% 预分配内存
CS_result.EachRunBestThresholds=zeros(RunTimes,TH_num);
CS_result.EachRunBestFitness=zeros(1,RunTimes);
CS_result.BestThresholds=zeros(1,TH_num);
CS_result.EachRunConvergenceTime=zeros(1,RunTimes);
CS_result.EachRunConvergenceFunCalNum=zeros(1,RunTimes);
CS_result.EachRunEveryIterBestFitness=zeros(RunTimes,T);
CS_result.EachRunEveryIterConvergenceTime=zeros(RunTimes,T);
%% 最外层循环，让CS跑RunTimes次
            ...
            while(~Success_FindBest_Index(RepeatTimes) && Iterates<=T)                 
                 %% STEP FOUR
                        ...
                        if (abs(fitness(LP,Each_Iterate_BestThresholds(Iterates,:))-BEST_EXHAUSTIVE_FITNESS)<=EPS)
                            CS_result.EachRunEveryIterConvergenceTime(RepeatTimes,Iterates)=toc;
...
%% 记录实验结果
    ...
    CS_result.EachRunEveryIterBestFitness(RepeatTimes,:)=1./Each_Iterate_BestFitness;
    [~,MinFitIndex]=min(Each_Iterate_BestFitness);
    CS_result.EachRunBestFitness(RepeatTimes)=fitness(LP,Each_Iterate_BestThresholds(MinFitIndex,:));
    CS_result.EachRunBestThresholds(RepeatTimes,:)=Each_Iterate_BestThresholds(MinFitIndex,:);
    CS_result.EachRunConvergenceTime(RepeatTimes)=CS_result.EachRunEveryIterConvergenceTime(RepeatTimes,MinFitIndex);
    CS_result.EachRunConvergenceFunCalNum(RepeatTimes)=EachRunFunCalNum(MinFitIndex);
...
%% 统计实验结果
    % 计算适应度值并统计
    [CS_result.Fitness.Max,CSFMax_Index]=max(CS_result.EachRunBestFitness);
    [CS_result.Fitness.Min,~]=min(CS_result.EachRunBestFitness);
    CS_result.Fitness.Mean=mean(CS_result.EachRunBestFitness);
    CS_result.Fitness.Variance=var(CS_result.EachRunBestFitness);
    CS_result.BestThresholds=sort(CS_result.EachRunBestThresholds(CSFMax_Index,:));
    % 计算“成功查找率”,“平均每次实验收敛时间”并统计
    CS_result.Success_Rate=sum(Success_FindBest_Index)/RunTimes;
    CS_result.MeanConvergenceTime=mean(CS_result.EachRunConvergenceTime);
    
disp('Statistic Metrics is Calculated !')

% 保存CS结果：ImageNmae_THChar_CS_result.mat
    FILENAME=strcat(Alg_Name,'_',ImageName,'_',TH_Char,'_CS_result.mat');
    save(FILENAME,'CS_result');     
        
disp('the Cuckoo Search is accomplised !!!')
