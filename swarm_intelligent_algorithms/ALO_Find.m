function  ALO_result=ALO_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
% function ALO_result=ALO_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
...
%    MAX_Iterations:
% Output:
%    ALO_result: is a 'Struct' containing the following result information
%        ALO_result.Fitness:
%            ALO_result.Fitness.Mean
%            ALO_result.Fitness.Variance
%            ALO_result.Fitness.Max
%            ALO_result.Fitness.Min
%        ALO_result.Success_Rate=sum(Success_FindBest_Num)/RunTimes;
%        ALO_result.BestThresholds: a vector containing 'TH_num' values
%        ALO_result.EachRunConvergenceTime:a vector containing 'RunTimes' values
%        ALO_result.MeanConvergenceTime: the mean convergence time

disp('the Ant Lion Optimizer is running...')

%% ALO STEPS
% STEP ONE
% Initialize the ALO parameters
...
%% 预分配内存
%记录结果
ALO_result.EachRunBestThresholds=zeros(RunTimes,TH_num);
ALO_result.EachRunBestFitness=zeros(1,RunTimes);
ALO_result.BestThresholds=zeros(1,TH_num);
ALO_result.EachRunConvergenceTime=zeros(1,RunTimes);
ALO_result.EachRunConvergenceFunCalNum=zeros(1,RunTimes);
ALO_result.EachRunEveryIterBestFitness=zeros(RunTimes,T);
ALO_result.EachRunEveryIterConvergenceTime=zeros(RunTimes,T);
%% 最外层循环，让ALO跑RunTimes次
    ...
    while(~Success_FindBest_Index(RepeatTimes) && Iterates<=T)        
        % iterations
            ...
            if (abs(fitness(LP,Each_Iterate_BestThresholds(Iterates,:))-BEST_EXHAUSTIVE_FITNESS)<=EPS)
                ALO_result.EachRunEveryIterConvergenceTime(RepeatTimes,Iterates)=toc;
...
%% 记录实验结果
    ...
    ALO_result.EachRunEveryIterBestFitness(RepeatTimes,:)=1./Each_Iterate_BestFitness;
    [~,MinFitIndex]=min(Each_Iterate_BestFitness);
    ALO_result.EachRunBestFitness(RepeatTimes)=fitness(LP,Each_Iterate_BestThresholds(MinFitIndex,:));
    ALO_result.EachRunBestThresholds(RepeatTimes,:)=Each_Iterate_BestThresholds(MinFitIndex,:);
    ALO_result.EachRunConvergenceTime(RepeatTimes)=ALO_result.EachRunEveryIterConvergenceTime(RepeatTimes,MinFitIndex);
    ALO_result.EachRunConvergenceFunCalNum(RepeatTimes)=EachRunFunCalNum(MinFitIndex);
end % End "for RepeatTimes=1:RunTimes"
    
disp('Statistic Metrics is Calculating...')
%% 统计实验结果
    % 计算适应度值并统计
    [ALO_result.Fitness.Max,ALOFMax_Index]=max(ALO_result.EachRunBestFitness);
    [ALO_result.Fitness.Min,~]=min(ALO_result.EachRunBestFitness);
    ALO_result.Fitness.Mean=mean(ALO_result.EachRunBestFitness);
    ALO_result.Fitness.Variance=var(ALO_result.EachRunBestFitness);
    ALO_result.BestThresholds=sort(ALO_result.EachRunBestThresholds(ALOFMax_Index,:));
    % 计算“成功查找率”,“平均每次实验收敛时间”并统计
    ALO_result.Success_Rate=sum(Success_FindBest_Index)/RunTimes;
    ALO_result.MeanConvergenceTime=mean(ALO_result.EachRunConvergenceTime);
    
disp('Statistic Metrics is Calculated !')

% 保存ALO结果：ImageNmae_THChar_ALO_result.mat
    FILENAME=strcat(Alg_Name,'_',ImageName,'_',TH_Char,'_ALO_result.mat');
    save(FILENAME,'ALO_result');     

disp('the Ant Lion Optimizer is accomplised !!!')
