function  PatternTypeTwo_CSO_result=PatternTypeTwo_CSO_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
% function PatternTypeTwo_CSO_result=PatternTypeTwo_CSO_Find(FunctionName,TH_num,RunTimes,MAX_Iterations)
...
%    MAX_Iterations:
% Output:
%    PatternTypeTwo_CSO_result: is a 'Struct' containing the following result information
%        PatternTypeTwo_CSO_result.Fitness:
%            PatternTypeTwo_CSO_result.Fitness.Mean
%            PatternTypeTwo_CSO_result.Fitness.Variance
%            PatternTypeTwo_CSO_result.Fitness.Max
%            PatternTypeTwo_CSO_result.Fitness.Min
%        PatternTypeTwo_CSO_result.Success_Rate=sum(Success_FindBest_Num)/RunTimes;
%        PatternTypeTwo_CSO_result.BestThresholds: a vector containing 'TH_num' values
%        PatternTypeTwo_CSO_result.EachRunConvergenceTime:a vector containing 'RunTimes' values
%        PatternTypeTwo_CSO_result.MeanConvergenceTime: the mean convergence time

disp('the Pattern Type Two Cat Swarm Optimization is running...')

%% Cat Swarm Optimization STEPS
    %% STEP ONE
        % Initialize the PatternTypeTwo_CSO parameters
...
%% 预分配内存
PatternTypeTwo_CSO_result.EachRunBestThresholds=zeros(RunTimes,TH_num);
PatternTypeTwo_CSO_result.EachRunBestFitness=zeros(1,RunTimes);
PatternTypeTwo_CSO_result.BestThresholds=zeros(1,TH_num);
PatternTypeTwo_CSO_result.EachRunConvergenceTime=zeros(1,RunTimes);
PatternTypeTwo_CSO_result.EachRunConvergenceFunCalNum=zeros(1,RunTimes);
PatternTypeTwo_CSO_result.EachRunEveryIterBestFitness=zeros(RunTimes,T);
PatternTypeTwo_CSO_result.EachRunEveryIterConvergenceTime=zeros(RunTimes,T);
%% 最外层循环，让PatternTypeTwo_CSO跑RunTimes次
...
for RepeatTimes=1:RunTimes
    ...
    %% STEP THREE
        %% the PatternTypeTwo_CSO main iterations
...
%                 CDC=round(((T-t+1)/T)*D); % the results do not improve with this strategy in the Low Dimensional Image
                ...
                % Find the best cat
                    ...
                    if (abs(fitness(LP,Each_Iterate_BestThresholds(Iterates,:))-BEST_EXHAUSTIVE_FITNESS)<=EPS)
                        PatternTypeTwo_CSO_result.EachRunEveryIterConvergenceTime(RepeatTimes,Iterates)=toc;
...
%% 记录实验结果
    ...
    PatternTypeTwo_CSO_result.EachRunEveryIterBestFitness(RepeatTimes,:)=1./Each_Iterate_BestFitness;
    [~,MinFitIndex]=min(Each_Iterate_BestFitness);
    PatternTypeTwo_CSO_result.EachRunBestFitness(RepeatTimes)=fitness(LP,Each_Iterate_BestThresholds(MinFitIndex,:));
    PatternTypeTwo_CSO_result.EachRunBestThresholds(RepeatTimes,:)=Each_Iterate_BestThresholds(MinFitIndex,:);
    PatternTypeTwo_CSO_result.EachRunConvergenceTime(RepeatTimes)=PatternTypeTwo_CSO_result.EachRunEveryIterConvergenceTime(RepeatTimes,MinFitIndex);
    PatternTypeTwo_CSO_result.EachRunConvergenceFunCalNum(RepeatTimes)=EachRunFunCalNum(MinFitIndex);
...
%% 统计实验结果
    % 计算适应度值并统计
    [PatternTypeTwo_CSO_result.Fitness.Max,PatternTypeTwo_CSOFMax_Index]=max(PatternTypeTwo_CSO_result.EachRunBestFitness);
    [PatternTypeTwo_CSO_result.Fitness.Min,~]=min(PatternTypeTwo_CSO_result.EachRunBestFitness);
    PatternTypeTwo_CSO_result.Fitness.Mean=mean(PatternTypeTwo_CSO_result.EachRunBestFitness);
    PatternTypeTwo_CSO_result.Fitness.Variance=var(PatternTypeTwo_CSO_result.EachRunBestFitness);
    PatternTypeTwo_CSO_result.BestThresholds=sort(PatternTypeTwo_CSO_result.EachRunBestThresholds(PatternTypeTwo_CSOFMax_Index,:));
    % 计算“成功查找率”,“平均每次实验收敛时间”并统计
    PatternTypeTwo_CSO_result.Success_Rate=sum(Success_FindBest_Index)/RunTimes;
    PatternTypeTwo_CSO_result.MeanConvergenceTime=mean(PatternTypeTwo_CSO_result.EachRunConvergenceTime);
disp('Statistic Metrics is Calculated !')

% 保存PatternTypeTwo_CSO结果：ImageNmae_THChar_PatternTypeTwo_CSO_result.mat
    FILENAME=strcat(Alg_Name,'_',ImageName,'_',TH_Char,'_PatternTypeTwo_CSO_result.mat');
    save(FILENAME,'PatternTypeTwo_CSO_result');      
        
disp('the Pattern Type Two Cat Swarm Optimization is accomplised !!!')
end
